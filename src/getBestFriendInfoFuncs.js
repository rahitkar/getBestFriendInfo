...
const getFriendsInfo = function(newInfo, name){
  if(Object.keys(newInfo).includes(name[0])){
    return [newInfo[name].bestFriend, newInfo[newInfo[name].bestFriend].father, newInfo[newInfo[name].bestFriend].mother, newInfo[newInfo[name].bestFriend].bestFriend];
  }
  return "not found";
};
